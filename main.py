...
try:
    ...
    try:
        # Fallback to different frequency, size, and channels
        pygame.mixer.init(frequency=22050, size=-16, channels=2)
    except pygame.error as e:
        print("Failed to initialize pygame mixer:", e)
        # Handle the error or continue without sound
            ...
            for handLms in results.multi_hand_landmarks:
                for id, lm in enumerate(handLms.landmark):
                    ...
                    if len(y) > 20:
                        ...
                        if (x[0] < x[9] < x[10] < x[11] < x[12]):
                            right_wait += 1
                        if (y[0] < y[11] and y[10] > y[11]):
                            rotate_wait += 1
                        if (y[0] > y[9] > y[10] > y[11] > y[12]):
                            down_wait += 1
...
pygame.display.set_caption('VIETNAM')
main_menu(win)
